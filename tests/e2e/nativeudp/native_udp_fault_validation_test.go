...
func validatePublicResourceAndKnockResourceIDWireDistinction(ctx context.Context, t *testing.T, httpTrap *lifecycleHTTPTrap) {
	...
	resourceBody := []byte(fmt.Sprintf(
		`{"errCode":"0","list":{"query":"connector_resource","version":1,"agent_id":"qurl-go-identity-wire-proof","connector_id":%q,"resource_public_key":%q,"crid":%q,"connector_routing_id":%q,"knock_resource_id":%q,"found_existing":false}}`,
