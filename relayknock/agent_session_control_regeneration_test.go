...
// the current NHP 2.0 envelope. It deliberately regenerates only the exact
...
func TestRegenerateAgentSessionControlVectors(t *testing.T) {
	...
	root["description"] = "Deterministic NHP 2.0 overload re-knock and strict exact-session retirement request/ACK vectors"
	root["notes"] = []string{
		...
		"NHP_RKN mixes the exact decoded 32-byte cookie into the ordinary NHP 2.0 header digest. NHP_EXT uses the ordinary no-cookie digest and carries the exact immutable receipt, never resource-scoped or bodyless exit authority.",
