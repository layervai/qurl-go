...
// The qURL v2 parser handles attacker-controlled input: the fragment
...
//
// Run locally with e.g. `go test -run=^$ -fuzz=FuzzDecodeB64Canonical -fuzztime=30s ./qurl`.
...
func FuzzParseFragment(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, fragment string) {
		...
		reassembled := qv2FragmentPrefix + "." + frag.ClaimsB64 + "." + frag.SecretB64 + "." + frag.SigB64
