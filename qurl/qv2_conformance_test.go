...
func assertTransportContract(t *testing.T, contract conformance.ConformanceTransportContract) {
	t.Helper()
	if contract.Prefix != transportPrefix || contract.CanonicalPrefix != qv2FragmentPrefix ||
		contract.ComponentMax != transportChunkLength || contract.MaxTransportLength != transportFragmentMaxLength ||
		contract.Fields.Claims.MaxEncodedLength != transportMaxClaimsLength ||
		contract.Fields.Claims.MaxChunks != transportMaxClaimsChunks ||
		contract.Fields.Secret.MaxEncodedLength != transportMaxSecretLength ||
		contract.Fields.Secret.MaxChunks != transportMaxSecretChunks ||
		contract.Fields.Signature.MaxEncodedLength != transportMaxSigLength ||
		contract.Fields.Signature.MaxChunks != transportMaxSigChunks {
		t.Fatalf("conformance transport contract does not match the qurl implementation: %+v", contract)
	}
}
...
func runSignatureClass(t *testing.T, cf *conformance.ConformanceFile) {
	...
	for _, v := range vf.Vectors {
		t.Run(v.Name, func(t *testing.T) {
			...
			case conformance.ExpectReject:
				...
				assertConformanceSignatureReject(t, v.RejectClass, verr)
			default:
...
func runSignatureTamper(t *testing.T, d *conformance.ConformanceTamperDerivation, pub *ecdsa.PublicKey, acceptClaimsB64 string, acceptRawSig []byte) {
	t.Run("payload_tamper", func(t *testing.T) {
		...
		if d.RejectClass != conformance.RejectClassTamper {
			t.Fatalf("tamper_derivation.reject_class = %q, want %q", d.RejectClass, conformance.RejectClassTamper)
		}
		if d.DeriveFrom != conformance.TamperDeriveFromAccept {
			t.Fatalf("tamper_derivation.derive_from = %q, this verifier only supports %q", d.DeriveFrom, conformance.TamperDeriveFromAccept)
		}
		if acceptRawSig == nil {
...
func applyClaimsTransform(name, claimsB64 string) (string, error) {
	switch name {
	case conformance.TamperTransformFlipFirstB64:
...
func assertConformanceSignatureReject(t *testing.T, rejectClass string, err error) {
	...
	case conformance.RejectClassHighS:
		want = errSignatureHighS
	case conformance.RejectClassWrongLength:
		want = errSignatureLength
	default:
		t.Fatalf("unexpected signature reject_class %q; conformance.ParseVectorFile should reject unknown classes before verification", rejectClass)
	}
...
func runClaimsParseClass(t *testing.T, class conformance.ConformanceClass) {
...
func runSecretParseClass(t *testing.T, class conformance.ConformanceClass) {
	...
	for _, v := range class.Vectors {
		t.Run(v.Name, func(t *testing.T) {
			...
			case conformance.ExpectReject:
				...
				if v.RejectClass == conformance.RejectClassKeyLength && !errors.Is(err, ErrKeyLength) {
...
func runStrictBase64Class(t *testing.T, class conformance.ConformanceClass) {
...
func runFragmentClass(t *testing.T, class conformance.ConformanceClass) {
	...
	for _, v := range class.Vectors {
		t.Run(v.Name, func(t *testing.T) {
			...
			case conformance.ExpectReject:
				...
				if v.RejectClass == conformance.RejectClassFragment && !errors.Is(err, ErrFragment) {
...
func runTransportClass(t *testing.T, class conformance.ConformanceClass) {
...
func runRelayAllowlistClass(t *testing.T, class conformance.ConformanceClass) {
...
func runServerIDClass(t *testing.T, class conformance.ConformanceClass) {
	requireNonEmpty(t, "server_id", class)
	for _, v := range class.Vectors {
		t.Run(v.Name, func(t *testing.T) {
			if v.Expect != conformance.ExpectAccept {
...
func assertParseOutcome(t *testing.T, v conformance.ConformanceVector, err error) {
	t.Helper()
	switch v.Expect {
	case conformance.ExpectAccept:
		if err != nil {
...
var allowedRejectClasses = map[string]map[string]struct{}{
	"transport":       {conformance.RejectClassTransport: {}},
	"claims_parse":    {conformance.RejectClassParse: {}},
	"secret_parse":    {conformance.RejectClassParse: {}, conformance.RejectClassKeyLength: {}},
	"strict_base64":   {conformance.RejectClassEncoding: {}},
	"fragment":        {conformance.RejectClassFragment: {}},
	"relay_allowlist": {conformance.RejectClassRelayURL: {}},
}
...
func assertRejectClassVocabulary(t *testing.T, cf *conformance.ConformanceFile) {
...
func requireNonEmpty(t *testing.T, name string, class conformance.ConformanceClass) {
	t.Helper()
