...
func TestLocalSignerNilGuards(t *testing.T) {
	var signer *LocalSigner
	if signer.KID() != "" {
		t.Fatal("nil signer returned a key id")
	}
	if _, err := signer.SignDigest(context.Background(), make([]byte, 32)); err == nil {
		t.Fatal("nil signer signed a digest")
	}
	if _, err := signer.PublicKeyDER(); err == nil {
		t.Fatal("nil signer returned a public key")
	}
}
