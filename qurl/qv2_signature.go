...
// callers MUST use VerifyLink instead.
//
...
// changes decoded byte 0 (the leading '{' of the claims JSON), so VerifyLink
...
// ErrSignature sentinel are a compatibility contract for downstream conformance
// tooling and must not change without a coordinated revision bump.
