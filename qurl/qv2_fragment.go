...
func parseFragment(fragment string) (*Fragment, error) {
	...
	if prefix != qv2FragmentPrefix {
		return nil, fmt.Errorf("%w: prefix must be %q, got %q", ErrFragment, qv2FragmentPrefix, prefix)
	}
	...
	if _, err := decodeB64(sigB64); err != nil {
...
func (f *Fragment) verify(ts *TrustStore) error {
	...
	sig, err := decodeB64(f.SigB64)
	if err != nil {
		return fmt.Errorf("sig part: %w", err)
	}
	return verifyRawSignature(pub, f.ClaimsB64, sig)
}

// VerifyLink extracts the fragment from a full qURL link and
...
func VerifyLink(qurlLink string, ts *TrustStore) (*Fragment, error) {
...
// decodeClaimsCellPublicKey returns the raw 32-byte X25519 cell public key from VERIFIED
...
func decodeClaimsCellPublicKey(c *Claims) ([]byte, error) {
...
func buildFragment(claimsB64, secretB64 string, rawSig []byte) (string, error) {
	...
	return strings.Join([]string{qv2FragmentPrefix, claimsB64, secretB64, encodeB64(rawSig)}, "."), nil
}
